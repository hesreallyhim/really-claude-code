...
However, the full power of agent teams is obscured by the official documentation. As one might expect to find in a product developed by a corporation, where hierarchy is taken as a matter of fact, agent teams are presented as hierarchical, both by design and by necessity. There is a "Team Lead" (the "Main Claude"), the sole agent with the creative power of Spawning other agents, and then there are the minions who are assigned tasks, and whom the Team Lead is charged with managing. Per the documentation: the Team Lead is "The main Claude Code session that creates the team, spawns teammates, and coordinates work". What results is a very powerful feature, but one where a "team" is more like the kind of team that you find in a corporate office, and not a team like an Olympic gold medal team, or the Power Rangers.
...
| `announcer` | Agent (haiku, gray) | Stateless multi-send relay -- fans out messages to multiple recipients |
| `pub-sub-relayer` | Agent (haiku, cyan) | Topic-based pub/sub relay -- forwards to channel subscribers from a file-backed registry |
| `/squad` | Command | Triggers squad formation for a complex task using the trio pattern |
| `/announce` | Command | Sends a message to multiple named recipients via the announcer |
| `/publish` | Command | Publishes a message to a pub/sub topic via the pub-sub-relayer |
| Cross-boundary warning | Hook | Warns (does not block) when agents message outside their designated sub-team |
