...
There's a peculiar blind spot in how Claude reasons about time. Ask it how long an engineering task will take and it will confidently answer "one to two days" — an estimate calibrated on human developers, story points, and eight-hour workdays. But Claude isn't a human developer; when _it_ executes the same task, the "writing the code" part that dominates a human's estimate collapses to minutes. The result is estimates that are wrong by one to three orders of magnitude, which makes planning _with_ agents nearly useless. This plugin does two things about that. First, a skill reframes every estimate into the part that actually collapses (agent execution time) versus the parts that don't (CI runs, human review, deploys, training) - _accurate, not merely smaller_. Second, and more in keeping with the theme above, it closes the epistemic gap the same way Context Awareness does: Claude's own `/goal` completions are already logged - with authoritative durations - right there in the session transcripts. A `SessionStart` hook mines them into a corpus of how long Claude's real tasks have _actually_ taken, pre-digests it into a compact per-bucket summary, and hands it back as a prior to reason from. Over time, the estimates stop being a borrowed human intuition and start being grounded in Claude's own measured history.
